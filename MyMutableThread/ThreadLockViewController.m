...
/**
 ...
 @synchronized 指令是在 Objective-C 代码中创建一个互斥锁非常方便的方法,它防止不同的线程在同一时间获取 同一个锁,然而在这种情况下,你不需要直接创建一个互斥锁或锁对象。相反,你 只需要简单的使用 Objective-C 对象作为锁的令牌, 如下，
 
如果你在多个不同的线程里面执行上述方法,每次在一个线程传递一个不同的对象给 obj1 参数,那么每次都将会拥有它的锁,并持续处理,中间不被其他线程阻塞；然而,如果你传递的是同一个对象,那么多个线程中的一个线程会首先获得该锁,而其￼他线程将会被阻塞直到第一个线程完成它的临界区。
...
注意，多线程的时候一定要用同一个锁，才能保证method执行完再执行其他method，不然就会出现method1执行了一半就执行method2了。
*/

- (void)lockForLockAndUnlockMethod {

    LockObj *obj1 = [LockObj shareInstance];
    NSGlobalLock *lock = [NSGlobalLock shareInstance];
    for (int i = 0; i < 3; i++) {
    //线程1
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
