...
//NSOperation本身有多核特性，在多线程下操作无需上锁。NSOperation在一个线程里面执行是并发的，当执行NSOperation的start时，会在它执行完之前返回，是因为调用start后NSOperation会创建一个线程去执行这个任务，此时无法确认该任务此时有没有执行，但是可以确认的是可以执行。
